...
#### Разбиение на конкретные задачи
...
- **Z8. События — выкинуть/перепридумать кнопку «Отметиться»**
  - Из `EventDetailsScreen` убрать текущую кнопку «Отметиться» или временно скрыть её за флагом.
  - Согласовать новый сценарий check‑in (авто по GPS, по старту пробежки, по таймеру) и зафиксировать его в отдельной секции `docs/changes/events-ux.md`.
  - После согласования — обновить вызовы `EventsService.checkInEvent` под новый сценарий.
...
План реализации: Новый фидбек (2026-02-12, продукт/UX)
 ...
 ---
 Z8. События — убрать кнопку «Отметиться»

 Текущее состояние: Кнопка в event_details_screen.dart:422-442, показывается при isParticipant == true. Использует
 hardcoded текст (не ARB ключи).

 Файлы:
 - mobile/lib/features/events/event_details_screen.dart

 Решение: скрыть кнопку из UI, оставить backend endpoint для будущего авто-check-in через Run flow.

 Изменения:
 - Закомментировать/удалить блок строк 422-442 в event_details_screen.dart
 - Оставить _onCheckIn метод и EventsService.checkInEvent для будущего использования
 - Создать docs/changes/events-ux.md с описанием решения

 Проверка: Открыть детали события как участник → кнопки «Отметиться» нет
