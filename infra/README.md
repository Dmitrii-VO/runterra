...
##### Mobile
- [ ] Во вкладке «Сообщения → Клуб» нет чата и нет поля ввода сообщения; надпись с названием клуба в этом месте не нужна.
- [ ] Во вкладке «Пробежка» при старте не отображается позиция и нет кнопки «Найти себя» (как в навигаторах); после пробежки темп считается некорректно/непрозрачно.
- [ ] После создания клуба «Пупкины» отображается наименование `Club new-club-id`, описание не соответствует введенному при создании, и создатель не является участником клуба.
- [ ] События, которые уже прошли, отображаются со статусом «Открыто».
