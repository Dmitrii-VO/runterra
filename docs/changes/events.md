...
- **Проблема:** На экране деталей события под «Точка старта» был серый placeholder с текстом «Карта (TODO)» и координатами.
- **Решение:**
  - Создан виджет `EventMiniMap` (`features/events/widgets/event_mini_map.dart`): Yandex MapKit карта 200px с CircleMapObject в точке старта, read-only (все жесты выключены), zoom 15.
  - По тапу на мини-карту — переход на `/map?lat=..&lon=..` (вкладка Карта с центрированием).
  - `MapScreen` расширен параметрами `focusLatitude`/`focusLongitude`; при их наличии карта центрируется на переданных координатах.
  - Маршрут `/map` в `app.dart` парсит query-параметры `lat`/`lon`.
  - Новый l10n ключ: `eventOpenOnMap`.
